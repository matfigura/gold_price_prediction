...
# metryki na TEŚCIE (nie CV)
mae_dt  = mean_absolute_error(y_test, y_pred_dt)
rmse_dt = np.sqrt(mean_squared_error(y_test, y_pred_dt))
r2_dt   = r2_score(y_test, y_pred_dt)

results.append({
    'model': result_dt['model'],   # spójna etykieta
    'MAE mean': mae_dt,
    'RMSE': rmse_dt,
    'R^2': r2_dt,
    'Best params': result_dt['Best params'],
    # opcjonalnie: dorzuć CV:
    # 'CV MAE': result_dt['CV MAE'],
    # 'CV RMSE': result_dt['CV RMSE'],
})
...
results.append({
    ...
    # opcjonalnie: "CV MAE": result_rf["CV MAE"],
})

plot_predictions(y_test, y_pred_rf, dates_test,
                 model_name="Random Forest",
                 save_path="results/random_forest_plot.png")

# ważności cech (zostaje bez zmian)
...
from src.data_lstm import create_lstm_data
X_train, X_test, y_train, y_test, dates_test, scaler_y, chosen = create_lstm_data()
start_time = time.time()
model_lstm, history_lstm = train_lstm(X_train, y_train, epochs=50, batch_size=32, val_split=0.1)
